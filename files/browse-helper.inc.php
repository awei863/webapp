...
function getAllPictures($connection) {
     try {
         $sql = 'select ImageID,Path, Title from imagedetails order by Title';
         $results = runQuery($connection, $sql, null);
         while ($paint = $results->fetch()) {
            echo '<li class="singlePicture">';
            echo '<div class="pictureDiv">'; 
            echo '<img class="displayPic" src="images/square150/'.$paint['Path'].'">';
            ...
            echo '<form action ="viewfavorites.php?fileName='.$paint['Path'].'&id='.$paint['ImageID'].'" method ="post">';
...
function getPicturesByCity($connection, $cityCode) {
     try {
         $sql = 'select ImageID,Path, Title from imagedetails where CityCode=? order by Title';
...
//         $search = $per.$search.$per;
         ...
         $sql = 'select ImageID,Path, Title from imagedetails where Title like "%'.$search.'%" order by Title';
