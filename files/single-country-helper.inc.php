...
function getPhotoCountries($connection){
   try {
   ...
   $statement = runQuery($connection,$sql, null);
    ...
    while ($gal = $statement->fetch()) {
        echo '<li><a href="single-country.php?iso='.$gal['ISO']. '">'.$gal['CountryName'].'</a></li>';
    }
    $connection = null;
   }
  ...
  function getCountries($connection){
   try {
   $sql = 'select CountryName, ISO from countries ORDER BY CountryName' ;
   $statement = runQuery($connection,$sql, null);
...
function getSearchCountries($connection, $search){
   try {
   $sql = 'select CountryName, ISO from countries  where CountryName like "%'.$search.'%" ORDER BY CountryName' ;
   $statement = runQuery($connection,$sql, null);
  ...
  function getContCountries($connection, $continent){
   try {
   $sql = 'select CountryName, ISO from countries where Continent=? ORDER BY CountryName';
   $statement = runQuery($connection,$sql, $continent);
    while ($gal = $statement->fetch()) {
        echo '<li><a href="single-country.php?iso='.$gal['ISO']. '">'.$gal['CountryName'].'</a></li>';
    }
    $connection = null;
